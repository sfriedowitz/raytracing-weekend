...
impl Perlin {
    pub fn new(n: usize) -> Self {
        Perlin {
            rand_vecs: Self::generate_rand_vecs(n),
            perm_x: Self::generate_perm_vec(n),
            perm_y: Self::generate_perm_vec(n),
            perm_z: Self::generate_perm_vec(n),
        }
    }
    ...
    fn noise(&self, point: Vec3) -> f64 {
        ...
        for di in 0..2 {
            for dj in 0..2 {
                for dk in 0..2 {
                    c[di][dj][dk] = self.rand_vecs[self.perm_x[(i + di) & 255]
    ...
    fn generate_rand_vecs(n: usize) -> Vec<Vec3> {
        let mut rng = rand::thread_rng();
    ...
    fn generate_perm_vec(n: usize) -> Vec<usize> {
        let mut p = Vec::with_capacity(n);
        for i in 0..n {
            p.push(i);
        }
